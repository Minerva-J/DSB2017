...
def load_scan(path):
    filename = os.path.join(path+'.mhd')
    # print('filename',filename)
    sliceim,origin,spacing,isflip = load_itk_image(filename)

        
    return sliceim,origin,spacing,isflip

def get_pixels_hu(slices):
    
    image = np.stack([slices[s] for s in range(slices.shape[0])])
    # Convert to int16 (from sometimes int16), 
    # should be possible as values should always be low enough (<32k)
    image = image.astype(np.int16)
    # hu_data = img.get_fdata()
    print('slices.shape',slices.shape,image.shape)
...
def step1_python(case_path):
    case,origin,spacing,isflip = load_scan(case_path)
    case_pixels = case
